...
class JiraLib:
    ...
    def get_all_epic(self,anddone=None):
        if anddone:
            qery='project = PROD  AND "Delivery team" = "%s" AND type = Epic'% (self.delivery_team)
        else:
            qery='project = PROD  AND "Delivery team" = "%s" AND status != Done AND type = Epic'% (self.delivery_team)
